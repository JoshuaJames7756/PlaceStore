// /api/admin/stores.js
// GET   — listar todas las tiendas con estado
// PATCH — activar/desactivar tienda manualmente
...
async function listarTiendas(req) {
  ...
  const stores = await sql`
    SELECT
      s.id, s.slug, s.store_name, s.location_city,
    ...
    WHERE (
      ...
      OR s.location_city ILIKE ${'%' + search + '%'}
...
async function toggleTienda(req) {
  ...
  const [store] = await sql`
    ...
    WHERE id = ${id}
    RETURNING id, store_name, is_active
  `;
