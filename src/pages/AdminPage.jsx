  ...
  return (
    <div className={styles.page}>
      <div className={styles.header}>
        <h1 className={styles.title}>Admin — PlaceStore</h1>
      </div>
          ...
          <div className={styles.payMain}>
            <div>
              ...
              <p className={styles.payMeta}>
                {p.location_city} · {p.method === 'tigo_money' ? 'Tigo Money' : 'Transferencia'}
              </p>
            </div>
          ...
          {stores.map(s => (
            <div key={s.id} className={styles.tableRow}>
              ...
              <span className={styles.cell}>
                <SubTag status={s.sub_status} expires={s.sub_expires_at} />
              </span>
...
function SubTag({ status, expires }) {
  ...
  const label = status === 'trial'  ? 'Trial'
              : status === 'active' ? `Activo hasta ${formatDate(expires)}`
              : 'Vencido';
  const cls   = status === 'active' ? styles.subTagActive
              : status === 'trial'  ? styles.subTagTrial
              : styles.subTagExpired;
